...
(defun update-item (item)
  (let ((new-item (copy-structure item)))
    (with-slots (name quality sell-in) new-item
      (if (and (not (equalp name "Aged Cheese"))
               (not (equalp name "Backstage passes to a concert")))
          (if (> quality 0)
              (if (not (equalp name "Fine Italian Silk"))
                  (setf quality (- quality 1))))
          (when (< quality 50)
            (setf quality (+ quality 1))
            (when (equalp name "Backstage passes to a concert")
              (if (< sell-in 11)
                  (if (< quality 50)
                      (setf quality (+ quality 1))))
              (if (< sell-in 6)
      ...
      (if (< sell-in 0)
          (if (not (equalp name "Aged Cheese"))
              (if (not (equalp name "Backstage passes to a concert"))
                  (if (> quality 0)
                      (if (not (equalp name "Fine Italian Silk"))
                          (setf quality (- quality 1))))
...
(defun run-gilded-rose (days)
  (write-line "OMGHAI!")
  (let* ((descriptions '(("Sports Memorabilia"                         10 20)
                         ("Aged Cheese"                                  2  0)
