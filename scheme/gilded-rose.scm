...
(define (update-quality items)
    (for-each
        (lambda (item)
            (if (and (not (string=? (item-name item) "Aged Cheese"))
                     (not (string=? (item-name item) "Backstage passes to a concert")))
                (if (> (item-quality item) 0)
                    (if (not (string=? (item-name item) "Fine Italian Silk"))
                        (item-quality-set! item (- (item-quality item) 1))))
                (cond ((< (item-quality item) 50)
                       (item-quality-set! item (+ (item-quality item) 1))
                       (if (string=? (item-name item) "Backstage passes to a concert")
            ...
            (if (< (item-sell-in item) 0)
                (if (not (string=? (item-name item) "Aged Cheese"))
                    (if (not (string=? (item-name item) "Backstage passes to a concert"))
                        (if (> (item-quality item) 0)
                            (if (not (string=? (item-name item) "Fine Italian Silk"))
                                (item-quality-set! item (- (item-quality item) 1))))
